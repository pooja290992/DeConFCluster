...
#         print('***EPOCH*** : ',epoch)
        ...
        data = []
#         temp = []
        for batch_idx, (vw1,vw2,vw3,vw4,vw5, labels) in enumerate(train_loader):
#             print('batch_idx : ', batch_idx)
#             for vw in views: 
            vw1, vw2, vw3,vw4, vw5, labels = map(lambda x: Variable(x), [vw1, vw2, vw3,vw4,vw5, labels])  
#                 temp.append(vw)
#             data.append(temp)
#             temp = []
            data = [vw1.to(device),vw2.to(device),vw3.to(device),vw4.to(device),vw5.to(device)]
...
#     print(scores_df.head())
    ...
    if not os.path.exists(results_file_name2):
        ...
        scores_df.to_csv(results_file_name2, mode = 'a', sep = ',', index = None)
