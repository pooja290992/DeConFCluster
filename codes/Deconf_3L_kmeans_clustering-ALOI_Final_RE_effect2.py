...
#         print('***EPOCH*** : ',epoch)
        ...
        data = []
#         temp = []
        for batch_idx, (vw1,vw2,vw3,vw4, labels) in enumerate(train_loader):
#             print('batch_idx : ', batch_idx)
#             for vw in views: 
            vw1, vw2, vw3, vw4, labels = map(lambda x: Variable(x), [vw1, vw2, vw3, vw4, labels])  
#                 temp.append(vw)
#             data.append(temp)
#             temp = []
            data = [vw1.to(device),vw2.to(device),vw3.to(device),vw4.to(device)]
...
def model_configs():
    ...
    n_epochs = [25]
    n_batch = [11025]
    lam_mu_km_list = [(0.0001,0.00001,0.5)]#[(0.01,0.0001,0.5)]#[(0.01,0.001,0.5)]#[(0.001,0.0001,0.5)]#[(0.001,0.00001,0.5)]#
    learning_rate_list = [5e-6]
    gamma = [1]
    ks_filters = [(4,5,8,3,16,3)]
    wd_list = [0.001]
    mpl_ks_list = [2]
...
res_base_path = base_path + 'Results/ablation/regularizers/lammu_reg/' + source + '_'
...
#     print(scores_df.head())
    ...
    results_file_name2 = res_base_path + 'res_dcfl3_' + loss_func + '_lammureg.csv'
